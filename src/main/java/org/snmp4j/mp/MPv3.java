...
public class MPv3
  ...
  public int prepareResponseMessage(int messageProcessingModel,
    ...
    if (logger.isDebugEnabled()) {
      // print this entry
      if (cacheEntry == null) {
        logger.debug("The cache entry is null for message - " + stateReference.getMsgID().getID());
      }
      else {
        logger.debug(String.format("Found cached entry for message=%s,  StateReference=%s", stateReference.getMsgID().getID(), cacheEntry));
      }
    }
