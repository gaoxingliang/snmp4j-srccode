...
public class USM extends SNMPv3SecurityModel {
  ...
  public int generateResponseMessage(int snmpVersion,
    ...
    if (securityStateReference != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The security ref is not null. and is - " + usmSecurityStateReference);
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The security ref is null.");
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(String.format("Found selected user engine=%s,sec=%s, user=%s", OctetString.fromByteArray(securityEngineID), OctetString.fromByteArray(securityName),
                user));
      }
