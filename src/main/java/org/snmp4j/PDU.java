...
public class PDU implements BERSerializable, Serializable {
  ...
  private static final LogAdapter logger = LogFactory.getLogger(PDU.class);
  ...
  public void setErrorStatus(int errorStatus) {
    ...
    // CALL ERROR STATUS SET
    if (errorStatus != 0) {
      logger.debug("Status set to " + errorStatus + " calling stacking trace :");
      StackTraceElement stackTraceElements [] = Thread.currentThread().getStackTrace();
      for (StackTraceElement e : stackTraceElements) {
        logger.debug(String.format("\t%s.%s#%d", e.getFileName(), e.getMethodName(), e.getLineNumber()));
      }
    }
